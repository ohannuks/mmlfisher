...
def get_images( phi_im ):
    ...
    n_img = len([key for key in phi_im.keys() if 'gw_0_img_center_x_' in key])
    # Get the image position parameters:
    x_img = jnp.array([phi_im['gw_0_img_center_x_%d' % i] for i in range(n_img)])
    y_img = jnp.array([phi_im['gw_0_img_center_y_%d' % i] for i in range(n_img)])
    return x_img, y_img
...
def get_gw_likelihood_params(kwargs_params, log_sigma_t, log_sigma_d, lens_model_list, fixed_parameters):
    ...
    kwargs_gw_likelihood['log_sigma_t'] = jnp.ones(3) * log_sigma_t
    kwargs_gw_likelihood['log_sigma_d'] = jnp.ones(4) * log_sigma_d
